...
Shader "Shader Forge/ProceduralSkybox" {
    Properties {
        ...
        _SunriseSunsetSize ("SunriseSunsetSize", Range(0, 2)) = 1.652064
...
////// Emissive:
                ...
                float uvHorizon = ((uv.r-saturate(pow(cos(((Day*node_8966)-(node_8966/2.0))),20.0)))+(uv*_SunriseSunsetSize_var));
                ...
                float3 emissive = (lerp(_HorizonColor_var.rgb,lerp(lerp(_SunriseSunsetColor_var.rgb,_MidNightColor_var.rgb,((1.0 - Day)+uvHorizon)),lerp(_SunriseSunsetColor_var.rgb,_MidDayColor_var.rgb,(Day+uvHorizon)),Day),uv)+(_LightColor0.rgb*pow(saturate((node_5759 + ( (dot((-1*lightDirection),viewDirection) - node_3529) * (0.0 - node_5759) ) / ((1.0 - (node_9175*node_9175)) - node_3529))),5.0)*_Sunintensity_var));
